      ...
      <body style="overflow-y: hidden;">
        <div class="wrapper row-offcanvas row-offcanvas-left">
          ...
          <aside class="right-side">                
			...
			<button type="submit" onclick="getCSVData()" class="btn btn-primary" ><i class="fa fa-bars"></i> Export Table Data</button>
			</form>
						...
						<table id="summary_table" class="table table-bordered table-hover">
							...
							<tr>
								...
								<td><?php if($row['agg_price']) echo $row['agg_price']; else echo '-'; ?></td>
								 <td><a href="#del_item" class="evtdltsnd fa fa-trash-o" style='text-decoration: none;color: #9A9CBB;'
                        data-itemid="<?php echo $row['item_id'];?>" data-serving="<?php echo $row['serve_id']; ?>" data-toggle="modal" data-target="#del_item"  style="color:white" ></a>
                      </td>
...
<!--script for data exporting-->

<script type="text/javascript" src="table2CSV.js"></script>

<div class="modal fade" id="del_item" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        ...
        <div class="modal-content" style="border-radius: 1em;">
            <div class="modal-header" style="background-color:#dd4b39; border-top-left-radius: 1em;
                  ...
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                      <h4 class="modal-title" id="myModalLabel" style="color:white;">Are You Sure ?</h4>
            </div>
			<form action="functions.php" method="post"> 
        <div class="modal-body" >
		<h4>
		 Removing this will remove all related information
		</h4>
		<input type="hidden" name="event" value="delete-item-serving">
		<input type="hidden" name="item" id="itemid" value=0>
		<input type="hidden" name="serving" id="serving1" value=0>
...
<script>
function getCSVData(){
var csv_value=$('#summary_table').table2CSV({delivery:'value'});
...
function toggleFlag(flag){
  ...
  var subcatid= $(this).data('itemid');
    var subserve= $(this).data('serving');
