...
<?php
  ...
  $sql="SELECT * FROM `tax` where venue_id=:vid";
  $sth=$conn->prepare($sql);
...
var levelCount=1;
   ...
   var tax_name=$(this).text();
	...
	var nxt='<div class="field col-md-12 alert alert-success alert-dismissable" style="border-left: 5px solid; margin-left:0px;" level="2" tax_id="'+taxId+'"> <input type="text" value="'+tax_name+'" name="'+tax_name+'" id="'+ taxId +'" class="field-name col-md-12">  </div>';
	$('#append_cat_text').append(nxt).fadeIn();
      $.each(data, function(index,value) {
  value.percentage= value.percentage?value.percentage:0;
       // alert(value.name); // will alert each value
        var field= "<div class='field col-md-12 well' level='2' tax_id='"+ taxId +"'  > <input type='text' value='" + value.name + "' id='" + value.id + "' name='categoryname' class='field-name col-md-6' readonly/> <input type='text' value='" + value.percentage + "' name='percentage' class='field-name col-md-2' /><span class='input-group-addon' style='display: inline; padding: 5px 10px 4px 10px; line-height:2;'>%</span> </div>";
 ...
 myfunction();
  ...
  var taxName=$(this).parents('#append_cat_text').find('.field:first-child input').val();
...
$("input:text[name=categoryname]").each(function(){
    ...
    function(data){
    console.log(data);
	var fselect='.field[tax_id=' + tax_id + ']';
	$(fselect).remove();
	 var field12= "<li class='field' level='1' tax_id=" + tax_id +"  ><a href='#'><span class='field-name'>" + taxName + "</span></a></li>";
    $('.tax_field_append').append(field12).fadeIn(1000);
    $('#append_cat_text').empty();
