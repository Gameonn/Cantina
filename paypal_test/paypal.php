<?php
require_once('../php_include/db_connection.php'); // include the db connection file
define('EMAIL_ADD', 'raju.prasad21-buyer@gmail.com'); // define any notification email
define('PAYPAL_EMAIL_ADD', 'jindal.ankit89-facilitator@gmail.com ');  // facilitator email which will receive payments change this email to a live paypal account id when the site goes live
...
switch($action){
	case "process": // case process insert the form data in DB and process to the paypal
		...
		$sth=$conn->prepare("INSERT INTO `purchases` (`invoice`, `product_id`, `product_name`, `product_quantity`, `product_amount`, `payer_fname`, `payer_lname`, `payer_address`, `payer_city`, `payer_state`, `payer_zip`, `payer_country`, `payer_email`, `payment_status`, `posted_date`) VALUES ('".$_POST["invoice"]."', '".$_POST["product_id"]."', '".$_POST["product_name"]."', '".$_POST["product_quantity"]."', '".$_POST["product_amount"]."', '".$_POST["payer_fname"]."', '".$_POST["payer_lname"]."', '".$_POST["payer_address"]."', '".$_POST["payer_city"]."', '".$_POST["payer_state"]."', '".$_POST["payer_zip"]."', '".$_POST["payer_country"]."', '".$_POST["payer_email"]."', 'pending', NOW())");
		$this_script = 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'];
		$p->add_field('business', PAYPAL_EMAIL_ADD); // Call the facilitator eaccount
		$p->add_field('personal', EMAIL_ADD);
...
';		echo '<div class="as_wrapper">';
	...
	case "ipn": // IPN case to receive payment information. this case will not displayed in browser. This is server to server communication. PayPal will send the transactions each and every details to this case in secured POST method by server to server. 
		...
		$sth1 = $conn->query($log_query);
		$log_check = $sth1->fetchAll();
		if(count($log_check) <= 0){
		
			$conn->query("INSERT INTO `paypal_log` (`txn_id`, `log`, `posted_date`) VALUES ('$trasaction_id', '$log_array', NOW())");
		}else{
		
			$conn->query("UPDATE `paypal_log` SET `log` = '$log_array' WHERE `txn_id` = '$trasaction_id'");
		} // Save and update the logs array
		$sth = $conn->query($log_query);
		$paypal_log_fetch 	= $sth->fetch();
		$paypal_log_id		= $paypal_log_fetch["id"];
		if ($p->validate_ipn()){ // validate the IPN, do the others stuffs here as per your app logic
		
		//echo $trasaction_id." ".$paypal_log_id." ".$payment_status." ".$invoice;
